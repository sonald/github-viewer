...
require('util').inherits(Repo, EventEmitter);
...
var GitData = (function() {
     ...
     * first parent. if no callback provided, emit commits one by one.
     */
    function getBranchCommits(ref, callback) {
        var self = this;
        var commits = [];

        var collect = callback ? function(tip) {
            commits.push(tip);

        }  : function(tip) {
            self.emit('branch.commit', tip);
        };
    ...
    function getFileHierachy(ref, callback) {
        ...
        this.getCommit(ref, function(err, commitobj) {
            self.getTree( commitobj.tree.sha, false, function(err, data) {
                debug(data);
                callback(err, data);
            });
        });
    ...
    function getTree(sha, recursive, callback) {
        var self = this;
